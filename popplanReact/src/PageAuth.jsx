...
function AuthPage() {
  const { setUser, setIsLoggedIn, setToken } = useContext(SessionContext); //設定容器與登入狀況
  // 控制目前是登入或註冊
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (resdata.status == 200) {
        ...
        // 安全地處理回傳資料
        if (resdata.data) {
          if (mode === 'login' && resdata.data.userCert) {
            setUser(resdata.data.userCert);
          }
          ...
          console.warn("⚠️ resdata.data 是 null");
        }
        setIsLoggedIn(true);
        console.log("***********3")
        // 可導向其他頁面，例如：

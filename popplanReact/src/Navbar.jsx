...
import { BsHeart, BsBell, BsPencilSquare, BsPlusSquare, BsClockHistory, BsFileEarmark } from 'react-icons/bs'; // 引入放大鏡圖示
...
function NavScrollExample() {
  ...
  const { user, isLoggedIn, setUser, setIsLoggedIn, token } = useContext(SessionContext); //共享容器
      ...
      <Container >
        ...
        <Navbar.Collapse id="navbarScroll">
          ...
          <Nav className="justify-content-end flex-grow-1 pe-3 gap-4 ">
            ...
            {/* 自己活動 */}
            <Nav.Link href="/myCreatedEvents" className='fs-5 navbar-bottom'>我的活動 <BsFileEarmark className='fs-4' /></Nav.Link>


            {/* 收藏報名 */}
            <Nav.Link href="/FavoriteEvent" className='fs-5 navbar-bottom'>
              <div className="d-flex align-items-center">
                <span className="me-2">收藏</span>
                {/* 包裹愛心 & badge 用來相對定位 */}
            ...
            <div className="d-flex align-items-center">
              {isLoggedIn ? (
                <>
                  <Nav.Link href="/PageUser" className='fs-5 navbar-bottom' >
                    <Image

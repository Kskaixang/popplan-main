...
function FavoriteEvent() {
  ...
  const EventURL = 'http://localhost:8080/event/favoriteEvent';
  ...
  const fetchEvent = async () => {
    try {
      ...
      if (resData.status == 200 && resData.data) { //查詢成功 由後端提取session 用回應送回來
        setFavoriteEvents(resData.data);
      } else { //非200 就維持  業務層錯誤
        setFavoriteEvents([]);
      }
  ...
  return (
    <>
      <Container className="mt-4 p-4 border border-gray bg-white shadow rounded">
        ...
        <EventListViewOneMission events={favoriteEvents} searchTerm={searchTerm} />

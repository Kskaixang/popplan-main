...
function CreateEventPage() {
  ...
  const { user } = useContext(SessionContext); //設定容器與登入狀況 通常有登入才能通過由路器
  const navigate = useNavigate(); //跳頁物件
  const [title, setTitle] = useState('這是一個默認的假活動標題這是一個默認的假活動標題這是一個默認的假活動標題這是一個默認的假活動標題這是一個默認的假活動標題這是一個默認的假活動標題');
  const [imageUrl, setImageUrl] = useState(null);
  const [imageFile, setImageFile] = useState(null); // 加這個
  const [description, setDescription] = useState('');
  const [location, setLocation] = useState('地址是給假活動用的地址是給假活動用的地址是給假活動用的');

...
import EventListViewOrder from "./components/EventListViewOrder";
...
function Order() {
  ...
  const EventURL = 'http://localhost:8080/registration/history';
  const [myHistorys, setMyHistorys] = useState([]);
  const fetchEvent = async () => {
    try {
      ...
      if (resData.status == 200 && resData.data) { //查詢成功 由後端提取session 用回應送回來
        setMyHistorys(resData.data);
        console.log(resData.data);  //有成功
      } else { //非200 就維持  業務層錯誤
        console.log('沒錯誤還一直空')
        setMyHistorys([]);
      }
    ...
    <>
      <Container className="mt-4 p-4 border border-gray bg-white shadow rounded">
        ...
        {isLoading ? (
          ...
          <EventListViewOrder registrations={myHistorys} searchTerm={searchTerm} />
        )}
